...
- [X] T006 运行基线构建和登录冒烟测试，记录 `docs/verification/baseline.md`。
...
- [X] T033 [P] [US2] 实现 `ruoyi-modules/ai-prompt/domain/PromptTemplateRenderer.java` 和变量、Schema 校验值对象。
- [X] T034 [US2] 实现 `ruoyi-modules/ai-prompt/application/PromptApplicationService.java`，完成 Prompt 创建、草稿版本和模板校验。
- [X] T035 [US2] 实现 `ruoyi-modules/ai-execution/domain/ExecutionRecorder.java` 和 `ExecutionCostCalculator.java`。
